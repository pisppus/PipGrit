...
namespace pipgrit
{
    const uint8_t BitmapFont::kGlyphs[BitmapFont::GlyphCount][BitmapFont::GlyphH] = {
    ...
    void BitmapFont::drawChar(uint16_t *buf, int16_t stride, int16_t bufH, int16_t x, int16_t y,
                              char ch, uint16_t fg, uint16_t bg, bool opaque) noexcept
    {
        ...
        for (int16_t c = 0; c < GlyphW; ++c)
        {
            ...
            const uint8_t colBits = glyph[c];
            for (int16_t r = 0; r < GlyphH; ++r)
            {
                const int16_t py = y + r;
                if (py < 0 || py >= bufH)
                    continue;

                const bool set = (colBits & (1 << r)) != 0;
                uint16_t *pixel = buf + static_cast<size_t>(py) * stride + px;
                if (set)
                    *pixel = fg;
                else if (opaque)
                    *pixel = bg;
    ...
    int16_t BitmapFont::drawString(uint16_t *buf, int16_t bufW, int16_t bufH,
                                   ...
                                   uint16_t fg, uint16_t bg, bool opaque) noexcept
    {
        ...
        {
            drawChar(buf, bufW, bufH, x, y, *p, fg, bg, opaque);
            x += Advance;
        }
        return x;
    }

    int16_t BitmapFont::textWidth(const char *text) noexcept
    {
        if (!text)
            return 0;
        size_t n = 0;
        while (text[n])
            ++n;
        return static_cast<int16_t>(n * Advance);
    }
}